...
**2026-01-04**: Fixed Android Bridge Mode duplicate shot sending. Three paths could send the same shot to GSPro: (1) native plugin via `sendShotToGSPro()`, (2) BridgeModeManager via `HandleGC2Shot()`, (3) GameManager via `RelayToGSPro()`. Fixed by removing Unity shot relay paths on Android with `#if UNITY_ANDROID && !UNITY_EDITOR` guards. Native plugin is now the ONLY source of GSPro communication on Android.
