...
**2026-01-04**: Fixed Android Bridge Mode notification and duplicate shot sending. Two issues: (1) Notification showed "GC2 Disconnected" despite shots being relayed - `sendShotFromIntent()` in GC2BridgeService.kt wasn't setting `isGC2Connected = true`; fixed by adding the flag before `updateNotification()`. (2) Duplicate shots sent to GSPro - three paths could send the same shot: native plugin via `sendShotToGSPro()`, BridgeModeManager via `HandleGC2Shot()`, and GameManager via `RelayToGSPro()`; fixed by removing Unity shot relay paths on Android with `#if UNITY_ANDROID && !UNITY_EDITOR` guards. Native plugin is now the ONLY source of GSPro communication on Android.
